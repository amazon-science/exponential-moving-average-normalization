...
parser.add_argument('--trainanno_x', default=None, type=str, metavar='PATH',
                    help='path to train annotation_x (default: None)')
parser.add_argument('--trainanno_u', default=None, type=str, metavar='PATH',
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    imagedir = os.path.join(args.data, 'JPEGImages')
    valanno = os.path.join(args.data, 'OI_Annotations/', args.valanno)
    if args.trainanno_x is not None and args.trainanno_u is not None:
        print("load annotations from {}/{}".format(args.trainanno_x, args.trainanno_u))
        trainanno_x = os.path.join(args.data, 'OI_Annotations/', args.trainanno_x)
        trainanno_u = os.path.join(args.data, 'OI_Annotations/', args.trainanno_u)
        train_dataset_x, train_dataset_u, val_dataset = get_imagenet_ssl(
            imagedir, trainanno_x, trainanno_u, valanno,
            weak_type=args.weak_type, strong_type=args.strong_type)
    else:
        print("load annotations from {} with {} percent".format(args.trainanno, args.anno_percent))
        ...
        train_dataset_x, train_dataset_u, val_dataset = get_imagenet_ssl_random(
            imagedir, trainanno, valanno, args.anno_percent, args.aug_type)
...
def get_imagenet_ssl(image_root, trainanno_x, trainanno_u, valanno,
    ...
    train_dataset_x = datasets.OpenImagesDataset(
        image_root, trainanno_x, transform=transform_x)

    train_dataset_u = datasets.OpenImagesDataset(
        image_root, trainanno_u, transform=transform_u)

    val_dataset = datasets.OpenImagesDataset(
        image_root, valanno, transform=transform_val)
...
def get_imagenet_ssl_random(image_root, trainanno, valanno, percent, train_type='DefaultTrain',
    ...
    base_dataset = datasets.OpenImagesDataset(image_root, trainanno)

    train_idxs_x, train_idxs_u = x_u_split(
        base_dataset.targets, percent, len(base_dataset.classes))

    train_dataset_x = datasets.OpenImagesDatasetWithIndex(
        image_root, trainanno, train_idxs_x,
        transform=transform_x)

    train_dataset_u = datasets.OpenImagesDatasetWithIndex(
        image_root, trainanno, train_idxs_u,
